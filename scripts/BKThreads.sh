...
sudo setsid iostat -x $DEVICE $report_interval_seconds > ${iostat_dir}/${4}_iostat &
IOPID=$!
sudo setsid $1 run rocksdb -s -P $2 -p rocksdb.optionsfile=$3 -p rocksdb.dir="${db_dir}/${4}" -p status.interval=$report_interval_seconds -threads 1 -target 1000 |& tee > ${report_dir}/${4}_report &
YCSB_PID=$!
wait "$YCSB_PID"
sudo pkill -9 iostat
wait "$IOPID"
...
do
	...
	for (( j=0; j<${#workloadsfiles[@]}; j++ ))
  	do
  		...
  		res_filename="${optionsfilename}_cores${cpucore}_${workloadfile}_r1"
