...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --loaddata)
            ...
            ;;
        --cache)
            if is_valid_cache "$2"; then
                cache="$2" # Assign the value of the next parameter to cache
                shift 2 # Shift to the next pair of parameters
            else
                echo "Error: --cache requires a valid cache size (e.g., 256MB, 1GB) or 'oscache'"
                exit 1
            fi
            ;;
        --ops_per_sec)
...
workloadsfiles=(
    ...
    # "workloada-90w10r-uniform"
    # "workloada-70w30r-uniform"
    # "workloada-50w50r-uniform"
    # "workloada-30w70r-uniform"
    # "workloada-10w90r-uniform"
	)

if [[ "$cache" == "oscache" ]]; then
    optionsfilenames=(
        # "OSCache_F4C12_MT2"
        # "OSCache_F32C96_MT2"
        "OSCache_F2C6_MT2"
    ...
    optionsfiles=(
        # "OSCache_F4C12_MT2.ini"
        # "OSCache_F32C96_MT2.ini"
        "OSCache_F2C6_MT2.ini"
    )
else
    optionsfilenames=(
        # "F4C12_MT2"
        # "CachePinL0_F4C12_MT2"
        # "F32C96_MT2"
        # "CachePinL0_F32C96_MT2"
        "F2C6_MT2"
    )
    optionsfiles=(
    	# "F4C12_MT2.ini"
     #    "CachePinL0_F4C12_MT2.ini"
     # "F32C96_MT2.ini"
        # "CachePinL0_F32C96_MT2.ini"
        "F2C6_MT2.ini"
    )
fi
...
optionsfile_dir="${optionsfiles_dir}/${optionsfiles[0]}"
optionsfilename=${optionsfilenames[0]}
	...
	for (( j=0; j<${#workloadsfiles[@]}; j++ ))
  	do
        ...
        ops_per_sec_k=$(($ops_per_sec / 1000))K
  		if [[ "$cache" == "oscache" ]]; then
    		res_filename="${optionsfilename}_cores${cpucore}_AA${ops_per_sec_k}_${workloadfile}_r1"
		else
    		res_filename="BlockCache${cache}_${optionsfilename}_cores${cpucore}_AA${ops_per_sec_k}_${workloadfile}_r1"
		fi
