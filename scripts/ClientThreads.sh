...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --threads)
            if is_integer "$2"; then
                threads="$2" # Assign the value of the next parameter to threads
                shift 2 # Shift to the next pair of parameters
            else
                echo "Error: --threads requires an integer value"
                exit 1
            fi
            ;;
        --loaddata)
...
do
	...
	for (( j=0; j<${#workloadsfiles[@]}; j++ ))
  	do
		...
		res_filename="CT${threads}_${optionsfilename}_cores${cpucore}_${workloadfile}_r1"
