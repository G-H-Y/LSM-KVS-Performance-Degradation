...
from globalvalues import XYLABEL_SIZE, XYTICKS_SIZE, TITLE_SIZE, LEGEND_SIZE
FIGPATH = PATH
...
def Draw_ReadInsert_QueueLatency_YCSB_ax(ax, report, LOG, addylabel=True, addxlabel=False):
    ...
    ax.plot(time_elapsed, avg_intended_insert_latency, color='#BEB8DC', label='avg_intended_insert_latency',linewidth=3.5)
    ax.plot(time_elapsed, avg_intended_read_latency, color='#82B0D2', label='avg_intended_read_latency',linewidth=3.5)

    # Adding vertical lines at intervals of 50 seconds
    for x in range(0, X_AXIS+1, STEPS):  # Adjust max value if needed
        ax.axvline(x=x, color='gray', linestyle='--', linewidth=1)

    # Format the y-axis in scientific notation
    ax.ticklabel_format(axis='y', style='scientific', scilimits=(3, 6))

    ax.tick_params(axis='both', labelsize=20)
    ax.legend(loc='upper right')
def Draw_ReadInsert_ProcessLatency_YCSB_ax(ax,report,LOG, addylabel=True,addxlabel=False):
    latency_info = ReadInsert_QueueLatency_YCSB(report)
    time_elapsed = latency_info['time_elapsed']
    ...
    ax.plot(time_elapsed, avg_write_latency, color='#BEB8DC', label='avg_write_latency',linewidth=3.5)
    ax.plot(time_elapsed, avg_read_latency, color='#82B0D2', label='avg_read_latency',linewidth=3.5)
...
def Draw_YCSB_BandwidthUsage_ax(ax,LOG,report,iostat,addtitle=False,figtitle=None,addylabel=True,addxlabel=False, legend=True):
    ...
    if addylabel:
        ax.set_ylabel('Bandwidth (MB/s)',fontsize=XYLABEL_SIZE, fontweight='bold')
    if addxlabel:
        ax.set_xlabel('Elapsed Time (Sec)', fontsize=XYLABEL_SIZE)
    ax.plot(time_elapsed, read_bandwidth, color='#8ECFC9', label="Read",linewidth=5)
    ax.plot(time_elapsed, write_bandwidth, color='#FFBE7A', label="Write",linewidth=5)
    ax.plot(time_elapsed, total_bandwidth, color='#FA7F6F', label="Total",linewidth=5)
    ax.axhline(y=1500, color='black', linestyle='--', label="Peak", linewidth=5)
    # ax.axhline(y=peak_write_bandwidth, color='#FFBE7A', linestyle='--', label="Peak Write Bandwidth Usage")
    # ax.axhline(y=peak_read_bandwidth, color='#8ECFC9', linestyle='--', label="Peak Read Bandwidth Usage")
    # # ax.axhline(y=avg_total_bandwidth, color='black', linestyle='dotted', label="Average Bandwidth Usage")
    # text = str(peak_bandwidth) + "MB/s"
    # ax.text(1, peak_bandwidth, text, fontsize=25, fontweight='bold')
    # text = str(peak_write_bandwidth) + "MB/s"
    # ax.text(250, peak_write_bandwidth, text, fontsize=25, fontweight='bold')
    # text = str(peak_read_bandwidth) + "MB/s"
    # ax.text(500, peak_read_bandwidth, text, fontsize=25, fontweight='bold')
    # ax.set_ylim((0, 2500))
    for x in range(XSTART, XEND+1, STEPS):  # Starts from 50, ends at 1000, steps by 50
        ax.axvline(x=x, color='gray', linestyle='--', linewidth=2.5)
    if legend:
        ax.legend(loc='upper left', fontsize=LEGEND_SIZE, ncol=2)
    # Modify tick parameters
    ax.tick_params(axis='both', labelsize=XYTICKS_SIZE, labelcolor='black', width=2)
    ax.set_xticks(range(400, 601, 50))
...
def Draw_YCSB_BandwidthAwait_ax(ax,LOG,report,iostat,addtitle=False,figtitle=None,addylabel=True,addxlabel=False):
    iocpu = iocpu_info(iostat)
    throughput_info = Throughput_YCSB(report)
    time_elapsed = throughput_info[0]
    ...
    if addylabel:
        ax.set_ylabel('R/W Requests Await (ms)',fontsize=20)
    if addxlabel:
        ax.set_xlabel('Elapsed Time (Sec)', fontsize=20)
    ax.plot(time_elapsed, r_await, color='#8ECFC9', label="Read Await",linewidth=3.5)
    ax.plot(time_elapsed, w_await, color='#FFBE7A', label="Write  Await",linewidth=3.5)
    for x in range(0, X_AXIS+1, STEPS):  # Starts from 50, ends at 1000, steps by 50
        ax.axvline(x=x, color='gray', linestyle='--', linewidth=1)
    ax.legend(loc='upper right')
    ax.tick_params(axis='both', labelsize=20)
...
def Draw_YCSB_CPUUtil_ax(ax,LOG,report,cpustat,addtitle=False,figtitle=None,addylabel=True,addxlabel=False, legend=True):
    ...
    if addylabel:
        ax.set_ylabel('CPU Util (%)',fontsize=XYLABEL_SIZE, fontweight='bold')
    if addxlabel:
        ax.set_xlabel('Elapsed Time (Sec)', fontsize=XYLABEL_SIZE, fontweight='bold')

    ax.plot(time_elapsed, cpu_util, color='red', label="CPU Utilization %",linewidth=5)
...
def Draw_ThroughputYCSB_withAVG_TailLatency_ax(ax, report, addtitle=False, addylabel=True, addxlabel=True, legend=True, rate=0):
    ...
    l = len(tpt_info[0])
    s = sum(tpt_info[1])
    tpt_info[0] = tpt_info[0][XSTART:XEND]
    tpt_info[1] = tpt_info[1][XSTART:XEND]
    avg_tpt = round(float(s / l), 2)

    print("AVG throughput: ", avg_tpt, "Ops/Sec")
    # if addylabel:
    #     ax.set_ylabel('Throughput (ops/s)', fontsize=XYLABEL_SIZE)
    if addxlabel:
        ax.set_xlabel('Elapsed Time (s)', fontsize=XYLABEL_SIZE, fontweight='bold')
    ax.plot(tpt_info[0], tpt_info[1], label="Real-time Throughput", linewidth=15)

    # Apply bold font style to tick labels
    for label in ax.get_xticklabels() + ax.get_yticklabels():
        label.set_fontweight('bold')
    # ax.axhline(y=avg_tpt, color='red', linestyle='--', label="Average Throughput", linewidth=8)
    ax.axhline(y=rate, color='orange', linestyle='--', label="Delayed Write Rate", linewidth=15)
    # ax.axhline(y=0, color='red', linestyle='--', label="zero throughput", linewidth=5)
    text = f'{int (rate/1000)}' + "K ops/s"
    ax.set_ylim((0, 80000))
    ax.set_yticks([0, 40000, 80000])
    # ax.text(XSTART, rate + 1000, text, fontsize=LEGEND_SIZE, fontweight='bold', bbox=dict(facecolor='white', alpha=1))
    # for x in range(XSTART, XEND + 1, STEPS):  # Starts from 50, ends at 1000, steps by 50
    #     ax.axvline(x=x, color='gray', linestyle='--', linewidth=2.5)
    ax.yaxis.set_major_formatter(FuncFormatter(format_tick))
    ax.tick_params(axis='both', labelsize=XYTICKS_SIZE, labelcolor='black', width=2)
    if legend:
        ax.legend(loc='upper left', fontsize=LEGEND_SIZE, handletextpad=0.5, handlelength=1)
    ax.tick_params(axis='both', which='both', direction='in', length=30, width=10, pad=25)
...
def Draw_Cmp_ax(ax, LOG, set_ylabel=True, set_xlabel=False):
    cmp_info = Compaction_duration(LOG)
    cmp_times = cmp_info[0]
    cmp_size = cmp_info[1]
    cmp_freq = cmp_info[2]
    cmp_sumsize = cmp_info[3]

    colors = ["orange", "red", "green", "purple", "black", "beige", "cyan", "magenta"]
    labels = ["L0L0", "L0L1", "L1L2", "L2L3", "L3L4", "L4L5", "L5L6", "L6L7"]
    for i in range(0, len(cmp_times)):
        x = np.array(cmp_times[i])
        y = np.array(cmp_size[i])
        ax.scatter(x, y, color=colors[i], s=len(cmp_times) - i)
        cmp_freq_tmp = int(len(cmp_times[i])/2)
        cmp_sumsize_tmp = int(sum(cmp_size[i])/1024) #GB
        linewidth = 1
        text = " Freq:{}; Size:{} GB".format(cmp_freq_tmp, cmp_sumsize_tmp)
        if i == 1:
            linewidth = 5
        for j in range(len(x)):
            if j % 2:
                ...
                y_range = [y[j - 1], y[j]]
                if j == 1:
                    ax.plot(x_range, y_range, linewidth=linewidth, color=colors[i], label=labels[i]+text)
                else:
                    ax.plot(x_range, y_range, linewidth=linewidth, color=colors[i])

    # ax.set_ylim((0, CMPSIZE_LIMIT))
    ax.set_yscale('log')
    # ax.set_ylim(bottom=0)
    # ax.set_yticks([0, 5000, 20000], ['0', '5k', '20k'])
    x_axis = []
    y_axis = []
    text = "Total Freq:{}; Size:{} GB".format(cmp_freq,cmp_sumsize)
    ax.scatter(x_axis, y_axis, color="black", label=text, marker="+")
    ax.legend(loc='upper right')

    if set_ylabel:
        ax.set_ylabel('Compaction Size (MB)', fontsize=20)
    if set_xlabel:
...
def Draw_YCSB_RTWSs_SlowStop_ax(ax, LOG, report):
    stall_info = extract_stall_data(LOG)
    ...
    for i in range(len(rdb_stallstop_sec)):
        stall_type = []
        for j in range(len(rdb_stallstop_sec[i])):
            stall_type.append(i + 1)
        x_axis = np.array(rdb_stallstop_sec[i])
        y_axis = np.array(stall_type)
        # labels[i] += " violation freq: {}<-[{},{}]".format(rdb_stall_freq[i]+rdb_stop_freq[i],rdb_stall_freq[i],rdb_stop_freq[i])
        # labels[i] += " Hit Freq: {}".format(rdb_stall_freq[i] + rdb_stop_freq[i])
        labels[i] += " Freq: {}".format(stall_info[i])
        ax.scatter(x_axis, y_axis, color=colors[i], label=labels[i])
    x_axis = []
    y_axis = []
    ...
    # text = "CC Violation Freq: {}<-[{},{}]".format(total,stall_freq,stop_freq)
    text = "Write Stall Freq: {}".format(stall_info[3])
    ax.scatter(x_axis, y_axis, color="black", label=text, marker="+")
    # text = "TD0 freq:{}; duration:{}s".format(ws, duration)
    # ax.scatter(x_axis, y_axis, color="black", label=text, marker="+")
    # text = "LowThroughput duration:{}s".format(near0_duration)
    # ax.scatter(x_axis, y_axis, color="black", label=text, marker="+")
    stall_info = extract_last_cumulative_stall_time(LOG)
    stall_text = "stall duration(sec): {}\n".format(stall_info[0]) + stall_info[1]
    ax.text(X_AXIS/2, 2, stall_text, fontsize=25, fontweight='bold')
    ax.legend(loc='upper right')
    ax.set_yticklabels([])
    ax.set_yticks([])
...
def Draw_L0fnum_ax(ax, LOG, addtitle=False, figtitle=None, addxlabel=False, addylabel=False, legend=True):
    ...
    ax.plot(time_elapsed, L0fnum, color='orange', label="L0 Count", linewidth=5)
    # ax.axhline(y=36, color='black', linestyle='dashdot', label="L0-CC Hard Threshold", linewidth=5)
    # ax.axhline(y=20, color='black', linestyle='dotted', label="L0-CC Soft Threshold",linewidth=5)
    ...
    if addtitle:
        ax.set_title(figtitle, fontsize=10)
    if addylabel:
        ax.set_ylabel('L0 Count', fontsize=XYLABEL_SIZE, fontweight='bold' )
    if addxlabel:
        ax.set_xlabel('Elapsed Time (s)', fontsize=XYLABEL_SIZE, fontweight='bold')
    for x in range(XSTART, XEND+1, STEPS):  # Starts from 50, ends at 1000, steps by 50
        ax.axvline(x=x, color='gray', linestyle='--', linewidth=2.5)
...
def Draw_PCsize_ax(ax, LOG, addtitle=False, figtitle=None, set_xlabel=False, set_ylabel=False):
    info = L0fnum_PS_info(LOG)
    time_elapsed = info[0]
    level0to1 = info[2]
    deeperlevel = info[3]
    ...
    ax.plot(time_elapsed, ps, color='purple', label="Estimated Pending Compaction Size", linewidth=3.5)
    ax.axhline(y=128, color='black', linestyle='dashdot', label="EC-CC Hard Threshold", linewidth=5)
    ax.axhline(y=64, color='black', linestyle='dotted', label="EC-CC Soft Threshold",linewidth=5)
    if addtitle:
        ax.set_title(figtitle, fontsize=10)
    if set_ylabel:
...
def Draw_TptYCSB_RTWSs_SlowStop_2ax(ax1, LOG, report, addtitle=False, figtitle=None, addylable=True, addxlable=True, addylable2=False, ylable2=None, legend=True, rate=0):
    Draw_ThroughputYCSB_withAVG_TailLatency_ax(ax1,report,addtitle, addylable, addxlable, legend, rate)
    # if addtitle:
    #     if figtitle!=None:
