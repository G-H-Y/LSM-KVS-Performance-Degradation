...
def Draw_ThroughputYCSB_withAVG_TailLatency_ax(ax, report, addtitle=False, addylabel=True, addxlabel=True, legend=True):
    ...
    ax.plot(tpt_info[0], tpt_info[1], label="Real-time Throughput", linewidth=8)
    ...
    # ax.set_xticks([0, 500, 1000])  # Set x-axis ticks at 0, 250, and 1000
    ax.set_yticks([0, 40000, 80000])  # Set y-axis ticks at 0, 40K, and 80K
    # ax.text(XSTART, avg_tpt + 1000, text, fontsize=LEGEND_SIZE, fontweight='bold', bbox=dict(facecolor='white', alpha=1))
    # for x in range(XSTART, XEND + 1, STEPS):  # Starts from 50, ends at 1000, steps by 50
    #     ax.axvline(x=x, color='gray', linestyle='--', linewidth=2.5)
...
def Draw_TptYCSB_RTWSs_SlowStop_2ax(ax1, LOG, report, addtitle=False, figtitle=None, addylable=True, addxlable=True, addylable2=False, ylable2=None, legend=True):
    ...
    # if addtitle:
    #     if figtitle!=None:
    #         ax1.set_title(figtitle, fontsize=TITLE_SIZE, fontweight='bold')
