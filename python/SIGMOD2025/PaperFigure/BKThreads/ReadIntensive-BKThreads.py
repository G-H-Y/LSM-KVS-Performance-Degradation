...
def Draw_Throughput_CCs(LOG_list, report_list, iostat_list,cpustat_list, path_list,figpath, figname,workload, cache,core, arrivalrate):
    ...
    row = 5
    fig, axes = plt.subplots(row, col, sharex='col', sharey='row', figsize=(80 * col, 30 * row))
    ...
    plt.subplots_adjust(wspace=0.015, hspace=0.04)
    plt.savefig(figpath + figname + ".jpg", bbox_inches='tight', dpi=fig.dpi, pad_inches=0.5)
    plt.show()
