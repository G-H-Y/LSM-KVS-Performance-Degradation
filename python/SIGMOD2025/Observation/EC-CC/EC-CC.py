...
def Draw_Throughput_CCs(LOG_list, report_list, iostat_list,cpustat_list, path_list,figpath, figname,workload, cache, arrivalrate, bkthreads):
    ...
    figtitle_list = ["EC-CC Threshold = 32G", "EC-CC Threshold = 64G", "EC-CC Threshold = 128G", "EC-CC Threshold = 256G", "EC-CC Threshold = 512G"]
...
if __name__ == '__main__':
    ...
    arrivalrate = 150
    bkthreads = "F32C96"
    dir = "cores128_ip{}".format(ip)
    workloads = ["100w0r","90w10r", "70w30r", "50w50r"]
    EXP_PATH = PATH + "EC-CC/{}/".format(dir)
    figpath = PATH + "EC-CC/fig/"
    LOG_list = []
    report_list = []
    iostat_list = []
    cpustat_list = []
    for workload in workloads:
        path_list = [
            EXP_PATH + "EC32G_F32C96_cores128_workloada-{}-zipfian_r1/".format(workload),
            EXP_PATH + "EC64G_F32C96_cores128_workloada-{}-zipfian_r1/".format(workload),
            EXP_PATH + "EC128G_F32C96_cores128_workloada-{}-zipfian_r1/".format(workload),
            EXP_PATH + "EC256G_F32C96_cores128_workloada-{}-zipfian_r1/".format(workload),
            EXP_PATH + "EC512G_F32C96_cores128_workloada-{}-zipfian_r1/".format(workload),
        ]
        LOG_list.clear()
        report_list.clear()
        iostat_list.clear()
        ...
        figname = "{}-{}-{}-ip{}".format(bkthreads, dir, workload,ip)
