...
def Draw_Throughput_CCs(LOG_list, report_list, iostat_list,cpustat_list, path_list,figpath, figname,workload, cache, arrivalrate, bkthreads):
    ...
    figtitle_list = ["MT # = 2","MT # = 8","MT # = 16","MT # = 32"]
...
if __name__ == '__main__':
    ...
    EXP_PATH = PATH + "MT-CC/{}/".format(dir)
    figpath = PATH + "MT-CC/fig/"
    LOG_list = []
    report_list = []
    iostat_list = []
    cpustat_list = []
    for workload in workloads:
        path_list = [
            EXP_PATH + "{}_MT2_cores128_workloada-{}-zipfian_r1/".format(bkthreads,workload),
            EXP_PATH + "{}_MT8_cores128_workloada-{}-zipfian_r1/".format(bkthreads, workload),
            EXP_PATH + "{}_MT16_cores128_workloada-{}-zipfian_r1/".format(bkthreads, workload),
            EXP_PATH + "{}_MT32_cores128_workloada-{}-zipfian_r1/".format(bkthreads, workload),
        ]
        LOG_list.clear()
        report_list.clear()
        iostat_list.clear()
