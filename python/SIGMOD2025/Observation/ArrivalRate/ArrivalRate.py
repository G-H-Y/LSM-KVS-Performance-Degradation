...
def Draw_Throughput_CCs(LOG_list, report_list, iostat_list,cpustat_list, path_list,figpath, figname,workload, cache,core, arrivalrate, bkthreads):
    ...
    figtitle_list = ["ArrivalRate = 10K ops/s", "ArrivalRate = 20K ops/s", "ArrivalRate = 30K ops/s", "ArrivalRate = 40K ops/s", "ArrivalRate = 50K ops/s", "ArrivalRate = 60K ops/s"]
    figtitle_list = ["ArrivalRate = 10K ops/s", "ArrivalRate = 20K ops/s", "ArrivalRate = 30K ops/s", "ArrivalRate = 40K ops/s", "ArrivalRate = 50K ops/s",]
    col = len(figtitle_list)
    row = 8
    fig, axes = plt.subplots(row, col, sharex='col', sharey='row', figsize=(15 * col, 6 * row))
    fig.suptitle("Setting: [BlockCache-{}GB, BKThreads-{}]  Workload: [{}-Zipfian] Hardware: [SSD, {}Core]".format(cache,bkthreads, workload, core), fontsize=80, fontweight='bold')
...
if __name__ == '__main__':
    ...
    for workload in workloads:
        print(workload)
        path_list = [
            EXP_PATH + "AA10K_{}_MT2_cores128_workloada-{}-zipfian_r1/".format(bkthreads, workload),
            EXP_PATH + "AA20K_{}_MT2_cores128_workloada-{}-zipfian_r1/".format(bkthreads, workload),
            EXP_PATH + "AA30K_{}_MT2_cores128_workloada-{}-zipfian_r1/".format(bkthreads, workload),
            EXP_PATH + "AA40K_{}_MT2_cores128_workloada-{}-zipfian_r1/".format(bkthreads, workload),
            EXP_PATH + "AA50K_{}_MT2_cores128_workloada-{}-zipfian_r1/".format(bkthreads, workload),
        ]
        LOG_list.clear()
        report_list.clear()
        iostat_list.clear()
