...
from globalvalues import CMPSIZE_LIMIT_TOTAL_GB, ARRIVAL_RATE, TOTAL_OPS, X_AXIS, AVG_LATENCY_LIMIT,STEPS
FIGPATH = PATH
...
def Draw_YCSB_BandwidthUsage_ax(ax,LOG,report,iostat,addtitle=False,figtitle=None,addylabel=True,addxlabel=False):
    iocpu = iocpu_info(iostat)
    ...
    if addylabel:
        ax.set_ylabel('I/O Bandwidth Usage (MB/s)',fontsize=20)
    if addxlabel:
        ax.set_xlabel('Elapsed Time (Sec)', fontsize=20)
    ax.plot(time_elapsed, read_bandwidth, color='#8ECFC9', label="Read Bandwidth Usage",linewidth=3.5)
    ax.plot(time_elapsed, write_bandwidth, color='#FFBE7A', label="Write Bandwidth Usage",linewidth=3.5)
    ax.plot(time_elapsed, total_bandwidth, color='#FA7F6F', label="Total Bandwidth Usage",linewidth=3.5)
    ax.axhline(y=peak_bandwidth, color='black', linestyle='--', label="Peak Bandwidth Usage")
...
def Draw_YCSB_CPUUtil_ax(ax,LOG,report,cpustat,addtitle=False,figtitle=None,addylabel=True,addxlabel=False):
    ...
    ax.plot(time_elapsed, cpu_util, color='red', label="CPU Utilization %",linewidth=3.5)
...
def Draw_ThroughputYCSB_withAVG_TailLatency_ax(ax, report, addtitle=False, addylabel=True, addxlabel=True):
    ...
    # total_ops = TOTAL_OPS
    # x_max = int(TOTAL_OPS/ARRIVAL_RATE)
    # ax.axhline(y=ARRIVAL_RATE, xmin=0, xmax=x_max, color='red', linestyle=':', linewidth=10, label="data arrival rate")
    ...
    ax.plot(tpt_info[0], tpt_info[1], label="throughput",linewidth=3.5)
    ...
    for x in range(0, X_AXIS+1, STEPS):  # Starts from 50, ends at 1000, steps by 50
        ax.axvline(x=x, color='gray', linestyle='--', linewidth=1)
    # ax.plot([0, x_max], [ARRIVAL_RATE, ARRIVAL_RATE], color='#FA7F6F', linewidth=5, label="data arrival rate")
    # ax.scatter([0, x_max], [ARRIVAL_RATE, ARRIVAL_RATE], color='#FA7F6F', marker='o',s=500)
...
def Draw_L0fnum_ax(ax, LOG, addtitle=False, figtitle=None, set_xlabel=False, set_ylabel=False):
    info = L0fnum_PS_info(LOG)
    time_elapsed = info[0]
    L0fnum = info[1]
    ax.plot(time_elapsed, L0fnum, color='orange', label="L0 SSTable Num", linewidth=3.5)
    ...
    if addtitle:
        ax.set_title(figtitle, fontsize=10)
    if set_ylabel:
        ax.set_ylabel('L0 SSTable Num', fontsize=20)
    if set_xlabel:
        ax.set_xlabel('Elapsed Time (Sec)', fontsize=XYLABEL_FONTSIZE)
    for x in range(0, X_AXIS+1, STEPS):  # Starts from 50, ends at 1000, steps by 50
        ax.axvline(x=x, color='gray', linestyle='--', linewidth=1)
...
def Draw_TptYCSB_RTWSs_SlowStop_2ax(ax1, LOG, report, addtitle=False, figtitle=None, addylable=True, addxlable=True, addylable2=False, ylable2=None):
    Draw_ThroughputYCSB_withAVG_TailLatency_ax(ax1,report,addtitle, addylable, addxlable)
    # if addxlable == False:
    #     ax1.set_xticklabels([])
    #     ax1.set_xticks([])
    # ax1.set_xlim((0,4500))
